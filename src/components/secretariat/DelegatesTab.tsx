...
export const DelegatesTab = ({ editionId, roleFilter = "delegate" }: { editionId: string; roleFilter?: RoleFilter }) => {
  ...
  const refresh = () => Promise.all([getRegistrations(editionId), getCommittees(editionId)])
    .then(([rAll, c]) => { setRegs(rAll.filter(r => r.role === roleFilter)); setCommittees(c); });
  ...
  const revokeEntry = async (id: string) => {
    ...
    refresh();
    toast({ title: "Entry revoked", description: "QR is no longer valid." });
  };
  const remove = async (id: string) => {
    if (!confirm("Delete this registration permanently?")) return;
    await deleteRegistration(id); refresh();
    toast({ title: "Deleted", description: "Portfolio is now available again." });
  };
  ...
  const exportCSV = () => {
    ...
    const isDel = roleFilter === "delegate";
    const headers = isDel
      ? ["ID","Name","Email","School","Grade","Phone","Final Committee","Final Portfolio","Pref1 Committee","Pref1 Portfolio","Pref2 Committee","Pref2 Portfolio","Verified","Created"]
      : ["ID","Name","Email","School","Grade","Phone","Committee","Role/Portfolio","Verified","Created"];
    const rows = regs.map(r => isDel ? [
      r.id, r.full_name, r.email, r.school, r.grade, r.phone,
      cName(r.committee_id), r.portfolio ?? "",
      cName(r.pref1_committee_id), r.pref1_portfolio ?? "",
      cName(r.pref2_committee_id), r.pref2_portfolio ?? "",
      r.payment_verified ? "YES" : "NO", r.created_at,
    ] : [
      r.id, r.full_name, r.email, r.school, r.grade, r.phone,
      cName(r.committee_id), r.eb_role ?? r.portfolio ?? "",
      r.payment_verified ? "YES" : "NO", r.created_at,
    ]);
    const csv = [headers, ...rows].map(r => r.map(c => `"${String(c ?? "").replace(/"/g, '""')}"`).join(",")).join("\n");
    const blob = new Blob(["\ufeff" + csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `${roleFilter}-${Date.now()}.csv`; a.click();
    URL.revokeObjectURL(url);
    toast({ title: "📦 Exported", description: `${regs.length} records.` });
  };

  const filtered = regs.filter(r => {
    const q = search.toLowerCase();
    const matchSearch = !q || r.full_name.toLowerCase().includes(q) || r.email.toLowerCase().includes(q) || r.school.toLowerCase().includes(q) || (r.portfolio ?? "").toLowerCase().includes(q);
    const matchFilter = filter === "all" || (filter === "verified" ? r.entry_verified_at != null : r.entry_verified_at == null);
  ...
  return (
    <div className="space-y-6">
          ...
          {(["all","pending","verified"] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={cn("px-4 py-2 text-xs font-semibold rounded-lg uppercase tracking-wider transition-all",
                filter === f ? "bg-gradient-primary text-primary-foreground" : "hover:bg-secondary")}>{f}</button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="glass rounded-3xl p-16 text-center"><Users className="w-12 h-12 text-muted-foreground mx-auto mb-3" /><p className="font-semibold">{ROLE_COPY[roleFilter].emptyHint}</p></div>
      ) : (
          ...
          {filtered.map(r => {
            ...
            const chosenPref =
              r.committee_id && r.portfolio
                ? r.pref1_committee_id === r.committee_id && r.pref1_portfolio === r.portfolio
                  ? 1
                  : r.pref2_committee_id === r.committee_id && r.pref2_portfolio === r.portfolio
                    ? 2
                    : 0
                : null;
            const roleBadge =
              ...
              <div key={r.id} className="glass-strong rounded-3xl p-5">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{r.school} · Grade {r.grade}</p>
                    <p className="text-xs text-muted-foreground truncate">{r.email} · {r.phone}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className={cn("text-[10px] font-bold tracking-widest px-2 py-1 rounded-full",
                      ...
                      {r.payment_status === "approved" ? "LOCKED"
                    ...
                    <span className="text-[10px] font-bold tracking-widest px-2 py-0.5 rounded-full bg-primary/10 text-primary">
                      {roleBadge}
                    </span>
                  </div>
                </div>
                ...
                {(r.pref1_committee_id || r.pref2_committee_id) && r.role === "delegate" && (
                ...
                <div className="flex flex-wrap gap-2 pt-3 border-t border-border">
                  ...
                  {r.payment_status === "approved" && !r.entry_verified_at && (
                    <Button variant="hero" size="sm" className="flex-1" onClick={() => generateEntryQR(r.id)}>
                      <QrCode className="w-3 h-3" /> Generate Entry QR
                    </Button>
                  )}
                  {r.entry_verified_at && (
      ...
      {viewing && (
        <div className="fixed inset-0 z-50 bg-foreground/70 flex items-center justify-center p-4" onClick={() => setViewing(null)}>
          <div className="glass-strong rounded-3xl p-6 max-w-2xl w-full" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4"><h3 className="font-bold">{viewing.name}'s ID</h3><Button variant="ghost" size="sm" onClick={() => setViewing(null)}>Close</Button></div>
...
const EditDelegateDialog = ({ reg, committees, onClose, onSaved }: {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (newPhoto) {
        id_image_path = await uploadIdImage(newPhoto);
      }
      const patch: any = {
      ...
      toast({ title: "✅ Saved", description: "Changes are live." });
      onSaved();
    } catch (e: any) {
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={e => setForm({ ...form, committee_id: e.target.value, portfolio: "" })}
          ...
          {isDelegate && (
            <>
              <div className="md:col-span-2 mt-2 pt-3 border-t border-border">
                <p className="text-xs uppercase tracking-widest text-muted-foreground font-bold mb-2">Preferences (editable)</p>
              </div>
              <div className="space-y-1.5">
                <Label>1st preference — committee</Label>
                <select
                  value={form.pref1_committee_id}
                  onChange={e => setForm({ ...form, pref1_committee_id: e.target.value, pref1_portfolio: "" })}
                  className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm"
                >
                  <option value="">— None —</option>
              ...
              <div className="space-y-1.5">
                <Label>1st preference — portfolio</Label>
                <select
                  value={form.pref1_portfolio}
                  onChange={e => setForm({ ...form, pref1_portfolio: e.target.value })}
                  className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm"
                >
                  <option value="">— Select —</option>
              ...
              <div className="space-y-1.5">
                <Label>2nd preference — committee</Label>
                <select
                  value={form.pref2_committee_id}
                  onChange={e => setForm({ ...form, pref2_committee_id: e.target.value, pref2_portfolio: "" })}
                  className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm"
                >
                  <option value="">— None —</option>
                  {committees.map(c => <option key={c.id} value={c.id}>{c.short_name}</option>)}
              ...
              <div className="space-y-1.5">
                <Label>2nd preference — portfolio</Label>
                <select
                  value={form.pref2_portfolio}
                  onChange={e => setForm({ ...form, pref2_portfolio: e.target.value })}
                  className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm"
                >
                  <option value="">— Select —</option>
...
const Stat = ({ icon: Icon, label, value, color = "primary" }: { icon: any; label: string; value: number; color?: "primary"|"success"|"warning" }) => (
  <div className="glass rounded-3xl p-5">
    <Icon className={cn("w-6 h-6 mb-2", color === "primary" && "text-primary", color === "success" && "text-success", color === "warning" && "text-warning")} />
    <div className="font-display text-3xl font-bold">{value}</div>
    <div className="text-xs text-muted-foreground tracking-wide">{label}</div>
  </div>
);

const PrefMini = ({ n, short, portfolio, chosen }: { n: 1 | 2; short?: string; portfolio?: string | null; chosen: boolean }) => (
  <div className={cn(
    "rounded-lg px-2 py-1.5 border text-[11px]",
    ...
    <div className="flex items-center justify-between gap-1">
      ...
      {chosen && <span className="text-[9px] font-bold tracking-widest text-success">✓</span>}
    </div>
    <div className="font-semibold truncate">{short ?? "—"}</div>
    <div className="text-muted-foreground truncate">{portfolio ?? "—"}</div>
  </div>
);
